    ...
    <section className="bg-off py-20 px-4 border-t border-border">
      <div className="max-w-5xl mx-auto">
        <RevealOnScroll>
          ...
          <p className="text-base text-muted-foreground max-w-xl leading-relaxed">אני לא חברה גדולה עם עשרות לקוחות — אני עובד עם מספר מוגבל של עסקים כדי לתת תשומת לב מלאה לכל אחד</p>
        </RevealOnScroll>
...
function FormSection() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    console.log("[v0] Starting EmailJS send with data:", formData)
    ...
    try {
      const result = await emailjs.send(
        ...
        "template_u9wpz5c", 
        {
          ...
          message: formData.notes || "לא צוין",
        },
        "cKWF3k5--EFenLoM6"
      )
